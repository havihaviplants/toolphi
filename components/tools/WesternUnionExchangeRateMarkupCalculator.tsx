...
export default function WesternUnionExchangeRateMarkupCalculator() {
  ...
  const [offeredRate, setOfferedRate] = useState<number>(0.97);
  ...
  const r = useMemo(() => {
    ...
    // Markup when offered is worse than mid (offered < mid in this convention)
    ...
    // Hidden FX cost in USD terms (how much value you lose vs mid)
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border p-4 space-y-2">
        <p className="text-sm opacity-80">
          This estimates <strong>Western Union exchange-rate markup</strong> by comparing the offered rate to the mid-market rate.
          Even when a transfer fee looks low, FX markup can be the real cost.
        </p>
        <p className="text-sm opacity-80">
          Enter rates as: <strong>1 USD → receiving currency units</strong> (e.g., 1 USD = 0.97 EUR).
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-1 text-xs opacity-70">
            The true market rate (often shown by Google or FX reference sites).
          </p>
        </div>

        <div>
          <label className="block text-sm font-medium">Western Union Offered Rate (1 USD →)</label>
          <input
            className="input"
            type="number"
            min={0}
          ...
          />
          <p className="mt-1 text-xs opacity-70">
            The rate shown by Western Union for your corridor/method.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border p-4 space-y-2">
        <p className="font-semibold">
          Exchange Rate Markup: {r.markupPercent.toFixed(2)}%
        </p>
        <p className="text-sm opacity-80">
        ...
        <p className="text-xs opacity-70">
          Tip: Total transfer cost = explicit fees + hidden FX cost. Use this markup estimate with your “charges” calculator.
        </p>
      </div>
    </div>
  );
