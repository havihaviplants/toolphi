...
export default function FreelancerTaxCalculator() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!Number.isFinite(netIncome) || netIncome < 0) {
      setError("Net freelance income must be a valid number (0 or greater).");
      return;
    }
    ...
    <div style={{ border: "1px solid #e1e4e8", borderRadius: 12, padding: 16 }}>
      <form onSubmit={handleSubmit}>
        <div style={{ display: "grid", gap: 12 }}>
          <div>
            <label>Annual net freelance income (after expenses)</label>
            <input
              name="netIncome"
      ...
      <div style={{ marginTop: 16 }}>
        {!result ? (
          <p>Enter values to estimate freelancer taxes and take-home pay.</p>
        ) : (
          <div style={{ display: "grid", gap: 6 }}>
            <div>
              Net freelance income: <strong>${money(result.netIncome)}</strong>
            </div>
            <div>
      ...
      <p style={{ marginTop: 10, fontSize: 13, color: "#333" }}>
        Note: This is a simplified estimator using flat rates you enter. Actual freelancer taxes
        depend on deductions, credits, brackets, and filing status.
      </p>
    </div>
  );
}
