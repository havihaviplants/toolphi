// components/tools/IcrPlanCalculator.tsx
"use client";
...
// ICR (simplified estimator):
// - Discretionary income ≈ AGI - 100% of poverty guideline (rough planning)
// - Payment ≈ 20% of discretionary income / 12  (common high-level approximation)
// Real ICR includes alternate payment calculation and caps; this tool is a planning estimator.

export default function IcrPlanCalculator() {
  ...
  const [povertyMultiplierPct, setPovertyMultiplierPct] = useState<string>("100");

  const [paymentPctOfDiscretionary, setPaymentPctOfDiscretionary] = useState<string>("20");

  // Optional: interest check
  ...
  const derived = useMemo(() => {
    ...
    // Minimal family-size adjustment (best accuracy: user pastes exact guideline)
  ...
  const handleReset = () => {
    ...
    setPovertyMultiplierPct("100");
    setPaymentPctOfDiscretionary("20");
    setLoanBalance("");
    setLoanApr("6.5");
    setResult(null);
  };
  ...
  return (
    <div>
      <p style={{ fontSize: 14, marginBottom: 12 }}>
        Estimate your student loan payment under a simplified{" "}
        <strong>ICR (Income-Contingent Repayment)</strong> model using discretionary income assumptions.
        This is a planning estimate — official ICR calculations can include alternate formulas and caps.
      </p>
      ...
      {result && (
        ...
        >
          ...
          <p>
            <strong>Estimated ICR monthly payment:</strong>{" "}
            {formatNumber(result.monthlyPayment)}
          </p>
          <p>
            <strong>Estimated ICR annual payment:</strong>{" "}
          ...
          <p style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
            ICR can be calculated differently depending on loan type and rules. Use this for
            planning, then verify with official servicer calculators.
          </p>
        </div>
      )}
    </div>
