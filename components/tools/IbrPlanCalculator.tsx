// components/tools/IbrPlanCalculator.tsx
"use client";
...
// IBR (simplified estimator):
// - Discretionary income ≈ AGI - 150% of poverty guideline
// - Payment rate often modeled as 10% or 15% of discretionary income depending on borrower/loan dates.
// This tool lets user choose 10% vs 15% explicitly.

export default function IbrPlanCalculator() {
  ...
  // Let user choose the typical IBR rate (10% or 15%)
  ...
  const derived = useMemo(() => {
    ...
    // Minimal family-size adjustment (best accuracy: paste exact guideline)
  ...
  return (
    <div>
      <p style={{ fontSize: 14, marginBottom: 12 }}>
        ...
        <strong>IBR (Income-Based Repayment)</strong> model using discretionary income assumptions.
        Choose the common IBR rate (10% or 15%) for planning. Official IBR rules (eligibility, caps, etc.)
        may differ.
      </p>
      ...
      <div style={{ display: "grid", gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <label style={{ fontSize: 14 }}>
            IBR rate (% of discretionary income)
            <select
            ...
            <div style={{ marginTop: 6, fontSize: 12, color: "#666" }}>
              Many references model IBR at 10% or 15% depending on borrower/loan timing. Use this as a planning toggle.
            </div>
          </label>
        </div>
      ...
      {result && (
        ...
        >
          ...
          <p>
            <strong>Estimated IBR monthly payment:</strong>{" "}
            {formatNumber(result.monthlyPayment)}
          </p>
          <p>
            <strong>Estimated IBR annual payment:</strong>{" "}
          ...
          <p style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
            IBR has additional rules (eligibility, payment caps vs standard plans, etc.). Use this as a planning estimate and verify with official sources.
          </p>
        </div>
      )}
    </div>
