...
export default function LongTermCapitalGainsTaxCalculator() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const longTermTaxRate = parseNumber(inputs.longTermTaxRate);
    ...
    if (!Number.isFinite(longTermTaxRate) || longTermTaxRate < 0) {
      setError("Long-term tax rate must be 0 or greater.");
      return;
    }

    const gain = salePrice - purchasePrice;
    const taxableGain = Math.max(gain, 0);
    const taxOwed = taxableGain * (longTermTaxRate / 100);
  ...
  return (
    <div style={{ border: "1px solid #e1e4e8", borderRadius: 12, padding: 16 }}>
      ...
      <div style={{ marginTop: 16 }}>
        {!result ? (
          <p>Enter values to calculate long-term capital gains tax.</p>
        ) : (
          <div style={{ display: "grid", gap: 6 }}>
            <div>
              Long-term gain: <strong>${money(result.gain)}</strong>
            </div>
            <div>
