...
export default function WesternUnionChargesCalculator() {
  const [amount, setAmount] = useState<number>(800);

  // WU fee schedule inputs (explicit charges)
  const [fixedFee, setFixedFee] = useState<number>(5);
  const [percentFee, setPercentFee] = useState<number>(1.2);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border p-4 space-y-2">
        <p className="text-sm opacity-80">
          Estimate <strong>Western Union explicit charges</strong> (fixed + percentage fees) with optional caps and promo discounts.
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          />
          <p className="mt-1 text-xs opacity-70">Flat service fee charged by Western Union.</p>
        </div>
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs opacity-70">Percent of amount charged as a fee (depends on corridor & method).</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border p-4 space-y-2">
        <p className="font-semibold">
          Western Union Charges: ${r.finalCharges.toFixed(2)}{" "}
        ...
        <p className="text-xs opacity-70">
          Note: Exchange-rate markup (spread) is not included here—use the Western Union FX markup calculator for hidden FX costs.
        </p>
      </div>
    </div>
  );
